...
public class LoginServerConnection extends Thread {
    ...
    public void CheckUser(String user, String pass){
           ...
           try {
               ...
               if (this.LOGIN_ID == 0)//incorrect username
               {
                   this.LOGIN_RESULT = 1;
               }
               else if (!this.pass.equals(this.LOGIN_PASSWORD))//incorrect password
               {
                   this.LOGIN_RESULT = 2;

               } 
               else if (this.LOGIN_BANNED == 1)//ban check
               {
                   this.LOGIN_RESULT = 3;
               }
               else if (this.LOGIN_ALLOG == 1)//session created check
               {
                   this.LOGIN_RESULT = 4;
               }
           } catch (Exception e) {
